...
class TherapistTodolistViewModel
    extends AsyncNotifier<List<TherapistTodolistModel>> {
  final TherapistTodolistRepository _repository = TherapistTodolistRepository();

  @override
  Future<List<TherapistTodolistModel>> build() async {
  ...
  Future<void> updateTodoConfirm(int id, bool isConfirm) async {
    final confirmTodo =
        state.value!
            .map(
              (todo) =>
                  todo.id == id ? todo.copyWith(isConfirm: isConfirm) : todo,
            )
            .toList();
...
final therapistTodolistViewModelProvider = AsyncNotifierProvider<
